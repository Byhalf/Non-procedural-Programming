...
solveSudoku' s (((x,y), m):ls)
  | length m == 1 = 
    if isValidMove s ((x,y), m)
      then  solveSudoku' (makeMove s ((x, y), [head m])) ls
    else sudokuMaker $ replicate 9 $ replicate 9 '1' -- or just make the invalid move but this is funner

  | isItValid s = s
  | isValidMove s ((x, y), [head m]) =
    let movedS = makeMove s ((x, y), [head m]) in
      if isItValid movedS then movedS else
        let fsudoku = solveSudoku' movedS ls in
